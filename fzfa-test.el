...
(ert-deftest fzfa-session-reentrant-snapshot-cannot-publish-obsolete-source ()
  ...
  (let* ((default-directory "/tmp/")
    ...
    (cl-letf (((symbol-function 'fzfa--session-api-p) (lambda () t))
      ...
      (should (= (fzfa-source-request-id src) 0))
      (should-not (fzfa-source-request-output src))
      (should-not (fzfa-source-request-snapshot-generation src)))))
...
(ert-deftest fzfa-helm-nested-same-result-buffer-has-distinct-owner ()
  ...
  (let* ((shared (generate-new-buffer " *fzfa shared Helm result*"))
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'fzfa-helm--ensure-loaded) #'ignore)
                  ((symbol-function 'fzfa-helm--active-minibuffer-buffer)
...
(ert-deftest fzfa-multi-render-legacy-final-output-remains-visible ()
  "The legacy combined API has no retained request-output object."
  (let* ((default-directory "/tmp/")
         (src (fzfa-make-source :spec '(:name "x" :command "fd")))
         (routes (make-hash-table :test #'equal))
         (output '(final ("alpha") 1 1)))
    (setf (fzfa-source-handle src) 'legacy-handle)
    (cl-letf (((symbol-function 'fzfa--session-api-p) (lambda () nil))
              ((symbol-function 'fzfa--multi-rank) (lambda (&rest _) 7)))
      (should (equal (fzfa--multi-render-async-output
                      src output 0 routes nil "a")
                     '("alpha")))
      (should (= (gethash "alpha" routes) 0))
      (should (eq (fzfa-source-last-async-output src) output))
      (should (= (fzfa-source-rank src) 7)))))
...
(ert-deftest fzfa-legacy-callouts-revalidate-source-ownership ()
  "Candidates, freshness, and stats callouts must not publish revoked work."
  (dolist (boundary '(candidates freshness stats))
    (let* ((old-handle (list 'old))
           (src (fzfa-make-source :command "ls"))
           (stats-called nil))
      (setf (fzfa-source-handle src) old-handle)
      (cl-labels ((revoke ()
                    (fzfa--source-clear-request src)
                    (setf (fzfa-source-handle src) (list 'replacement))))
        (cl-letf (((symbol-function 'fzfa--session-api-p) (lambda () nil))
                  ((symbol-function 'fzf-native-async-candidates)
                   (lambda (&rest _)
                     (when (eq boundary 'candidates) (revoke))
                     (unless (eq boundary 'freshness) '("obsolete"))))
                  ((symbol-function 'fzf-native-async-result-fresh-p)
                   (lambda (&rest _)
                     (when (eq boundary 'freshness) (revoke))
                     t))
                  ((symbol-function 'fzf-native-async-stats)
                   (lambda (&rest _)
                     (setq stats-called t)
                     (when (eq boundary 'stats) (revoke))
                     '(1 . 1))))
          (should (eq (fzfa--source-async-out src "q" 10) t))
          (when (memq boundary '(candidates freshness))
            (should-not stats-called)))))))
...
(ert-deftest fzfa-legacy-nested-query-owns-render-publication ()
  "A nested legacy query must prevent the outer query from publishing last."
  (let* ((source (fzfa-make-source :spec '(:name "x" :command "fd")))
         (routing (make-hash-table :test 'equal))
         inside-rank
         outer-result)
    (setf (fzfa-source-handle source) 'legacy-handle)
    (cl-labels
        ((render (query)
           (fzfa--multi-render-async-output
            source (fzfa--source-async-out source query 10)
            0 routing nil query)))
      (cl-letf (((symbol-function 'fzfa--session-api-p) (lambda () nil))
                ((symbol-function 'fzf-native-async-candidates)
                 (lambda (_handle query _limit) (list query)))
                ((symbol-function 'fzfa--final-p) (lambda (&rest _) t))
                ((symbol-function 'fzf-native-async-stats)
                 (lambda (&rest _) '(1 . 1)))
                ((symbol-function 'fzfa--multi-rank)
                 (lambda (&rest _)
                   (unless inside-rank
                     (setq inside-rank t)
                     (unwind-protect (render "new")
                       (setq inside-rank nil)))
                   9)))
        (setq outer-result (render "old"))))
