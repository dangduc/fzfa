    ...
    (unwind-protect
        (cl-letf (((symbol-function 'active-minibuffer-window)
          ...
          (with-current-buffer nested
            (setq fzfa--minibuffer-session nested-session))
...
(ert-deftest fzfa-preview-current-rechecks-minibuffer-owner ()
  "A candidate read that opens another picker must cancel old preview."
  (let* ((window (selected-window))
         (original (window-buffer window))
         (outer (generate-new-buffer " *fzfa preview outer*"))
...
(ert-deftest fzfa-helm-session-token-includes-origin-buffer ()
  "Nested Helm sessions differ even when they reuse one result buffer."
  (let* ((result-buffer (generate-new-buffer " *fzfa Helm result*"))
         (outer-origin (generate-new-buffer " *fzfa Helm outer*"))
         (nested-origin (generate-new-buffer " *fzfa Helm nested*"))
         (helm-alive-p t)
         (helm-buffer result-buffer)
         (helm-current-buffer outer-origin)
         (outer-token (fzfa-helm--current-session-token)))
    (unwind-protect
        (progn
          (setq helm-current-buffer nested-origin)
          (should-not (equal outer-token
...
(ert-deftest fzfa-helm-poller-updates-only-claimed-session ()
  ...
  (let* ((result-buffer (generate-new-buffer " *fzfa Helm poll result*"))
         (outer-origin (generate-new-buffer " *fzfa Helm poll outer*"))
         (nested-origin (generate-new-buffer " *fzfa Helm poll nested*"))
         (helm-alive-p t)
         (helm-buffer result-buffer)
         (helm-current-buffer outer-origin)
         (helm-pattern "")
         (helm-map (make-sparse-keymap))
         poll candidates stop updates)
    (unwind-protect
        (cl-letf (((symbol-function 'fzfa-helm--ensure-loaded) #'ignore)
                  ...
                  ((symbol-function 'fzfa--source-async-candidates)
                   (lambda (&rest _) nil))
                  ((symbol-function 'helm-force-update)
                   (lambda () (push helm-current-buffer updates))))
          (pcase-let ((`(,source . ,stop-fn)
                       (fzfa-helm--async-source-and-stop
            ...
            (setq helm-current-buffer outer-origin)
            (funcall poll)
            (should (equal updates (list outer-origin)))
            (funcall stop)
            (setq stop nil)))
      (when stop (funcall stop))
      (kill-buffer result-buffer)
...
(ert-deftest fzfa-helm-preview-timer-rejects-nested-session ()
  "A queued preview must not read a nested Helm selection."
  (let* ((result-buffer (generate-new-buffer " *fzfa Helm preview result*"))
         (outer-origin (generate-new-buffer " *fzfa Helm preview outer*"))
         (nested-origin (generate-new-buffer " *fzfa Helm preview nested*"))
         (helm-alive-p t)
         (helm-buffer result-buffer)
         (helm-current-buffer outer-origin)
         (fzfa-preview-delay 0.1)
         (token (fzfa-helm--current-session-token))
         (owner-p (lambda ()
                    (equal token (fzfa-helm--current-session-token))))
         callback selections previews)
    (unwind-protect
        (cl-letf (((symbol-function 'run-with-idle-timer)
                   (lambda (_delay _repeat function &rest args)
                     (setq callback (lambda () (apply function args)))
                     'preview-timer))
                  ...
                  ((symbol-function 'fzfa--preview-call)
                   (lambda (&rest args) (push args previews))))
          (funcall (fzfa-helm--make-debounced-preview-fn nil owner-p) nil)
          (should callback)
          (setq helm-current-buffer nested-origin)
          (funcall callback)
...
(ert-deftest fzfa-helm-producer-is-not-fired-during-construction ()
  ...
  (let* ((calls 0)
    ...
    (cl-progv '(helm-map helm-pattern helm-alive-p
        ...
        (list test-helm-map "" t (current-buffer) (current-buffer))
      (cl-letf (((symbol-function 'fzfa-helm--ensure-loaded) #'ignore)
                ((symbol-function 'helm-make-source)
...
(ert-deftest fzfa-session-request-epoch-blocks-equal-id-aba-result ()
  "Reusing a handle and request ID does not restore revoked ownership."
  (let* ((default-directory "/tmp/")
...
(ert-deftest fzfa-session-failure-report-rechecks-request-owner ()
  "A source restart during failure reporting revokes the old status."
  (let* ((default-directory "/tmp/")
         (src (fzfa-make-source :command "old"))
         (old-handle 'old-handle)
         (replacement-handle 'replacement-handle)
    ...
    (cl-letf (((symbol-function 'fzfa--session-api-p) (lambda () t))
              ...
              ((symbol-function 'fzf-native-async-snapshot)
               (lambda (&rest _)
                 (cl-incf snapshots)
                 (error "obsolete request was materialized")))
              ((symbol-function 'message)
               (lambda (&rest _)
                 (cl-incf messages)
                 (fzfa-source--restart src "replacement" #'ignore)))
