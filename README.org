...
per-source narrowing, preview, and apply.

~consult--multi~ is sync only; ivy/counsel has no multi-source
primitive (~counsel-buffer-or-recentf~ concatenates two lists into a
flat collection, no per-source semantics); helm has native
multi-source but re-shells per keystroke on async sources, which
...
portion appears editable — which matches the old async UX where the
producer was fixed and only filtering was interactive.  Press ~>~
inside the minibuffer to cycle through three display modes:
...
- *vertico/consult* has =consult--multi=, but it's sync only — which
  potentially limits the scope of what you can do with a multi source
  function.  =consult-buffer= shows the shape of what's possible.
