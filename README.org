...
Optional integrations live in the ~extensions/~ subdirectory.  They are
opt-in via ~fzf-async-extensions~ — a list of short symbols that the
lazy setup walks: for each ~SYMBOL~ it ~require~ s ~fzf-async-SYMBOL~
and calls ~fzf-async-SYMBOL-setup~ if defined.  ~extensions/~ is added
to ~load-path~ on first use so no manual path wiring is needed.
Extension loading happens the first time you invoke any fzf-async
command (or up front if you call ~fzf-async-setup~ explicitly).
...
| ~chrome~    | ~fzf-async-chrome~     | none              | ~fzf-async-chrome-bookmarks~, ~fzf-async-chrome-refresh~ — open Chromium-family bookmarks via ~browse-url~                          |
...
| ~fzf-async-extensions~         | ~(pass spotlight music chrome company mail notmuch)~ | Extensions to load from ~extensions/~ during setup. |
