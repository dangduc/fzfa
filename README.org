...
Source-column ordering is stable by default: columns appear in the order
declared in ~fzfa-find-any-commands~ / ~fzfa-find-some-commands~ via the
~fzfa-src-idx~ text property that ~fzfa-multi-read~ stamps on each
candidate.
