...
Helm support lives in ~fzfa-helm.el~, auto-required during ~fzfa-setup~
via ~(with-eval-after-load 'helm …)~ — install helm, enable
~helm-mode~, and existing ~fzfa-*~ commands route through helm
sources backed by ~fzf-native~ scoring.  No user configuration is
needed; each fzfa helm handler let-binds ~helm-completion-style~ to
~'emacs~ for the duration of its session so helm-mode defers
matching to ~completion-styles~ (where fzfa's style is registered).
The user's global ~helm-completion-style~ is left untouched.
...
*** Full setup example
To get a completion experience where each ~helm~ source matches using a
consistent algorithm (e.g. ~fzf-native~), use ~fussy~ for non-~fzfa~ sources
and ~fzfa~ otherwise. Example below.
...
#+begin_src emacs-lisp :tangle yes
  ...
  (use-package helm
    ...
    :config
    (helm-mode)
    ;; Setting `helm-completion-styles' to 'emacs allows `helm' to use `fussy'.
...
| ~fzfa-preview-display-action~  |    reuse existing window | `display-buffer' action used by `fzfa-preview-show' (see note below).         |
...
~fzfa-preview-display-action~ is the `display-buffer' action used by
~fzfa-preview-show~ (which every built-in handler funnels through).
Default tries `display-buffer-reuse-window' first (so successive
previews land in the same window slot) and falls back to
`display-buffer-use-some-window' (an existing window; never splits).
Net effect: 1 window — uses it (preview replaces whatever was there);
2+ — `get-lru-window' picks the least-recently-used window, normally
the non-selected one.

Set to any other `display-buffer' action to customize placement —
for example `(display-buffer-in-side-window (side . right) (window-width . 0.5))'
for a dedicated right side window, or `(display-buffer-below-selected
(window-height . 0.4))' to always split a fresh window below the
current one.  See the variable's docstring for the common
`display-buffer-*' function set.
