...
single source receives the full budget.  A widened session divides the budget
between its sources and truncates the final merge as a second safety check.
...
fzfa session.  A widened multi-source session divides this budget between
sources.  A narrowed or single-source session gives the full budget to its
active source.  Fzfa also truncates the merged list to this value.
