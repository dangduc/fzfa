...
The shell command in a ~-2p~ session can be long (e.g. ~rg
--line-number --no-heading --with-filename %s ''~), and most of the
time you only want to type into the quoted-argument slot.  Each ~-2p~
minibuffer therefore starts in *compact* mode: flags are hidden by default.

#+begin_example
  Compact:   #rg ... '|'#
  Expanded:  #rg --line-number --no-heading --with-filename %s '|'#
#+end_example

If there is no quoted-argument slot in the command, the entire flag
tail is hidden silently (e.g. ~find %s -type f~ shows as ~#find#~).

Press ~<~ inside the minibuffer to toggle between compact and
expanded.

Customize ~fzfa-2pass-compact-key~ to change the key, or set it to
~nil~ to disable the feature entirely (no binding, no initial
compaction — the full command is shown verbatim, as before).
...
| ~fzfa-smart-find~    | ~fzfa-smart-find-2p~     | ~fzfa.el~         |
| ~fzfa-smart-grep~    | ~fzfa-smart-grep-2p~     | ~fzfa.el~         |
...
#+begin_src emacs-lisp :tangle yes
  (use-package fzfa
    ...
    :custom (fzfa-2p-functions '(fzfa-ag-2p fzfa-ag-files-2p fzfa-fd-2p
                                 ...
                                 fzfa-locate-2p fzfa-rg-2p fzfa-smart-find-2p
                                 fzfa-smart-grep-2p fzfa-ugrep-2p))
    :init
    ...
    (autoload 'fzfa-smart-find-2p "fzfa"        nil t)
    (autoload 'fzfa-smart-grep-2p "fzfa"        nil t))
#+end_src
...
(rather than reimplementing it), it composes transparently with both
multi-source and two-pass dispatch:

- *Multi-source* — drop ~fzfa-smart-find~ into ~fzfa-find-any-commands~
  (or any other list passed to ~fzfa-multi-read~) and the multi-source
  machinery extracts the chosen backend's ~:command~ / ~:category~ /
  ~:group~ as if it were that backend directly.  No special-casing.
- *Two-pass* — wrapping with ~fzfa-2p-define~ produces
  ~fzfa-smart-find-2p~ / ~fzfa-smart-grep-2p~ automatically; the
  two-pass dispatcher extracts the active backend's shell command,
  lets you edit it, and routes the selection back through the smart
  wrapper.  Both are gated on ~fzfa-2p-functions~ alongside the other
  ~-2p~ variants — see [[*Two-pass configuration & autoloading][Two-pass configuration & autoloading]].
...
|------------------------------+----------------------------------------------------------------------------------------------|
| ~fzfa-async-completing-read~ | =:dynamic-collection t= binding, =ivy-push= closure on timer/retry, stats via =ivy-pre-prompt-function= |
| ~fzfa-sync-completing-read~  | =:dynamic-collection t= binding only (sync has no streaming or stats overlay)                          |
| ~fzfa-2pass-completing-read~ | =:dynamic-collection t= + =ivy-push-2pass= closure mirroring the cmd-split + restart logic of the =('t)= body |
...
vertico / icomplete consult, so multi-source sessions render as a
flat merged list by default.  ~fzfa-ivy.el~ registers a display
transformer under ivy's ~t~ fallback caller that prepends
~[source-name] ~ to each candidate carrying a fzfa tofu suffix.

The face for the label is ~fzfa-multi-source-label~ (inherits
...
~fzfa-extensions~ (which includes ~helm~ by default).  Loading the
file alone is a no-op; ~fzfa-helm-setup~ — called from ~fzfa-setup~ —
defers handler registration via ~(with-eval-after-load 'helm …)~ so
the four handler defvars get populated only when helm itself is
actually loaded.
...
| Entry point                  | Handler var               |
|------------------------------+---------------------------|
| ~fzfa-async-completing-read~ | ~fzfa-async-helm-handler~ |
| ~fzfa-sync-completing-read~  | ~fzfa-sync-helm-handler~  |
| ~fzfa-2pass-completing-read~ | ~fzfa-2pass-helm-handler~ |
| ~fzfa--multi-read~           | ~fzfa-multi-helm-handler~ |
  ...
  (defun my/helm-mini ()
    ...
    (helm :sources
          (append helm-mini-default-sources
                  ...
                  (fzfa-helm-source-from-command 'fzfa-vcs-any))
...
~helm-mode~ it dispatches through ~fzfa-multi-helm-handler~ with the
...
| Smart fuzzy scoring                 | ✓ (C)                      | ×                  | ×                 | ×             | ×                 | ✓ (binary) |
| Multi-component query               | ✓ (fzfa)                   | ✓ (orderless)      | ✓ (substring AND) | ✓ (regex AND) | ✓ (via orderless) | ✓ (fzf)    |
| Producer runs once                  | ✓                          | ✓                  | ×                 | ×             | ×                 | ✓          |
...
- There may be some fuzzy matching packages that try to interop with ~helm~ or ~counsel~ though it's hard to say how performant they are.
- From a quick eye test, ~fzfa~ was the most performant around handling large datasets/sources with ~fzf.el~ and ~affe~ behind.
- Multicomponent queries exist with these packages with varying functionality. For example, using multicomponent matching is possible
with ~orderless~, but adding ~flex~ filtering to the matching will slow Emacs to a crawl.
...
~fzf~ scores bonuses for word boundaries, camel-case transitions,
consecutive runs, and position (leading chars matter more).  Typing
=fhf= matches =fzfa-helm-flymake= with a higher score than
=fzfa-find-foo= because of the word-boundary alignment.
...
- =orderless= treats query components as substring / literal / regex
  ...
  candidate sets. -The advantage of this approach is it can integrate with
  other types of sorting Emacs provides like history/recency sorting.-
- =affe= filters via =all-completions= + orderless regex.  Pure
  filter; no per-character scoring; ranking is whatever
  =all-completions= returns.
- =helm-= sources use helm's multi-pattern matcher (substring or
  regex), there are a few external packages that tries to hook smart
  fuzzy matching in ~helm~ but they have the listed downsides mentioned
  above.

~fzfa~ runs fzf scoring in C via
[[https://github.com/dangduc/fzf-native][fzf-native]] with the same
algorithm as the =fzf= command-line tool — including the extended
syntax (=^foo=, =foo$=, ='foo=, ==foo==, =!foo=).

** Multi-source

- *helm* has native multi-source (helm-mini-style: stream from N
  producer processes concurrently, each filtered/scored live).
- *vertico/consult* has =consult--multi=, but it's sync only — which potentially
  limits the scope of what you can do with a multi source function.
  =consult-buffer= shows the shape of what's possible.
...
Emacs packages like ~helm~, ~counsel~ and ~consult~ are somewhat
tied to their base packages e.g. ~helm~, ~ivy~, and ~vertico~ / plain
~completing-read~. ~consult~ and ~vertico~ are
the most -standard- in the sense that they integrate the most with base
Emacs but users generally aren't able to use a ~consult~
function with ~helm~'s UI or vice versa. This is why these packages
(and this one too) generally duplicate the same async-* functions.
e.g. There will be a helm-rg, counsel-rg, consult-rg.

~fzfa~ tries to integrate with these packages on the other hand.
Each frontend gets a dedicated extension (=fzfa-helm.el=, =fzfa-ivy.el=,
=fzfa-vertico.el=). Using ~helm~ as an example, ~fzfa~ can provide "sources"
that ~helm~ can read (e.g. with ~helm-mini~) or calling ~fzfa-*~ functions
using ~helm~'s UI.
...
- *=helm-=* (helm-rg, helm-find-files, etc.): mature, comprehensive
  ...
  different completion algorithms which is good or bad depending on wh
  you ask.
...
- *=ivy/counsel-=* (counsel-rg, counsel-fzf, etc.): also mature, comprehensive
  ...
  (e.g. ~completion-styles~) and development has slowed, it prevents a tricky
  value proposition at this point. From my testing, the most typing
  responsive of the bunch (no benchmarking).

- *=vertico/consult-=*: newest of bunch, but mature/comprehensive at this point
  The various ~consult-*~ integrates heavily with Emacs base packages so
  it's probably the best in terms of a future-looking experience. The
  ~all-completions~ and ~completion-regexp-list~ approach it takes to filtering
  means it "just works" with most packages as well as ~orderless~ but that's the
  same reason it's not yet? compatible with ~fzf~ style matching, which mostly
  wants a clean/empty prefix to match against. It gets around this somewhat using
  a 2 pass approach (e.g. ~consult-rg~), where you can set a specific ripgrep
  shell command to run and a separate query to filter.
