...
|---------------------------+------------+----------------------------------------|
| ~fzfa-find-hungry~   | fd / find  | Files under all derived directories    |
| ~fzfa-swiper-hungry~ | rg / grep  | Line content under all derived dirs    |
...
~(fzfa-buffer fzfa-recent-file fzfa-find-hungry)~.  Add
any other arg-less ~fzfa~ command to that list and it shows up as
...
#+begin_src emacs-lisp :tangle yes
  (defun my/fzfa-files-only ()
    (interactive)
    (fzfa-multi-read
     '(fzfa-recent-file fzfa-find-hungry fzfa-locate)
     :prompt "files: "))
#+end_src
...
Default value of ~fzfa-extensions~ is ~(pass spotlight music chrome company mail notmuch)~.
...
| ~fzfa-extensions~         | ~(pass spotlight music chrome company mail notmuch)~ | Extensions to ~require~ during setup. |
...
~fzfa-project-shell-command~ does the same from the project root.
