...
single source receives the full budget.  A widened session allocates fair
shares in declared source order.  Completed sparse sources donate unused slots
to later sources, while unfinished async sources reserve their full share.
Fzfa truncates the final merge as a second safety check.
