...
The persistent shell-command session API is available on macOS, Linux, and
FreeBSD.  The bundled Windows module provides batch scoring, so fzfa
~:candidates~ sources work there, but streamed ~:command~ sources signal a
clear ~user-error~ before a session starts.

Large-pool latency and memory depend on candidate length, query shape, match
density, host hardware, and the configured result limit.  Benchmark the
producer and matcher with the workload you intend to use; the comparison below
describes architecture, not a universal throughput ranking.
...
[[https://github.com/dangduc/fzf-native][fzf-native]] (provides the C scoring module).
...
On every changed query, each active source is filtered again.  Sync sources
use ~fzf-native-score-all~.  A literal list is fetched once and remains
untagged while it is scored.  Fzfa applies the source's presentation budget
before it copies candidates and records source routes.

Async sources submit one request, poll metadata, and materialize one
authoritative snapshot through the request-aware session API.  Stable redraws
reuse that output.  The producer, such as ~find~, ~fd~, or ~ripgrep~, stays
alive and continues to add candidates for native incremental retries.

~fzfa-max-candidates~ is one session-wide presentation budget.  A narrowed or
single source receives the full budget.  A widened session repeatedly
water-fills fair shares against each source's full filtered demand.  Completed
sparse sources donate unused slots to dense sources in either direction, while
unfinished async sources reserve a fair share.  Demand evidence persists while
a larger native LIMIT request is pending, so donated capacity converges instead
of oscillating across redraws.  Fzfa truncates the final merge as a second
safety check.
...
~fzfa-helm-make-sync-source~.  ~helm-pattern~ is split into (CMD . FILTER).
CMD goes to the producer's INPUT slot and refires only when CMD changes.
FILTER scores the latest raw snapshot with ~fzf-native-score-all~.

Fzfa classifies producer kind from function arity.  It does not fire the
producer as a construction probe.  A synchronous callback updates the current
pass.  An asynchronous callback updates the owned snapshot and schedules
~helm-force-update~.  Failed deliveries release their input for retry and
cannot publish through a retained stale callback.  The ~:display~ argument
(~hidden~, ~compact~, or ~full~) controls how CMD appears in the minibuffer.
...
For ~:command~ sources, a polling timer checks request-aware snapshot metadata
and calls ~helm-force-update~ only after a new publication.  A stable redraw
reuses the previous materialized result.  The multi handler uses one shared
timer for all source handles.  It limits updates to one per
~fzfa-input-throttle~ and skips them when ~input-pending-p~ is non-nil.  Thus,
keystrokes take priority over producer-driven refreshes.
...
| Variable                       | Default                  | Description                                                                   |
|--------------------------------+--------------------------+-------------------------------------------------------------------------------|
| ~fzfa-max-candidates~          | 10000                    | Session-wide candidate presentation budget (see note below).                  |
...
With a non-nil cap, the native reader normalizes each record incrementally. It
retains at most N decoded characters before the newline arrives. ANSI payloads
and discarded suffix bytes do not increase the partial-record buffer. Excluded
bytes do not enter the candidate arena or the scorer. A nil cap permits an
unbounded producer record.
...
~fzfa-max-candidates~ is the maximum number of candidates presented by one
fzfa session.  A widened multi-source session starts with deterministic fair
shares, then gives unused capacity from each completed sparse source to later
sources.  Pending work reserves its full share, so simultaneous output cannot
overcommit the cap.  A narrowed or single-source session gives the full budget
to its active source.  Fzfa also truncates the merged list to this value.

For command sources, the limit is passed to the native request before its
snapshot crosses into Elisp.  For list and producer sources, fzfa keeps the raw
snapshot untagged, scores it, applies its budget, and tags only the visible
subset.  Literal lists are fetched once per session.  The limit reduces Elisp
copying, route-table memory, and frontend rendering.  It does not limit the raw
producer pool or guarantee that matching work stops after N candidates.
...
Streamed ~:command~ sources require the persistent fzf-native session API,
which is currently available on macOS, Linux, and FreeBSD.  On Windows, use
a list or producer through ~:candidates~; the native batch scorer remains
available there.
...
- ~affe~, ~helm~, and ~consult~ can take advantage of
  ...
  single-threaded.
- ~fzfa~ and ~fzf.el~ both keep a producer alive while the query changes, but
  they use different UIs and integration boundaries.  Measure them with your
  candidate distribution before drawing a throughput conclusion.
...
- *[[https://github.com/minad/affe][affe]]*: closest architectural
  ...
  of the Emacs plumbing. Filtering is single-threaded Elisp, which avoids
  ~fzfa~'s native dependency.  ~fzfa~ learned from this project's background
  producer + filter flow.  The packages use different matching algorithms and
  should be measured with the intended workload.
