...
| ~flymake~   | ~fzfa-flymake~    | none (built-in)   | ~fzfa-flymake~ — jump to a Flymake diagnostic; with prefix arg, gather diagnostics from every buffer in the current project; modeled on ~consult-flymake~ |
