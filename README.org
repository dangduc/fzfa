...
On every keystroke each source is re-filtered — sync sources via
~fzf-native-score-all~, async sources via ~fzf-native-async-candidates~
against the live producer pool. The producer (e.g. ~find~, ~fd~, ~ripgrep~)
does not reset and continues generating results we can score/filter against.
...
| ~fzfa-replay-file~               | On-disk path (default ~~/.emacs.d/.fzfa-replay~)           |
...
(ag chrome company eglot emacs embark evil fd find firefox flymake git grep
 ...
 project regexp rg safari shell spotlight transient ugrep vertico vc)
#+end_src
